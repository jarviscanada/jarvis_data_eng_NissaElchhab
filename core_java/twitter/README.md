...
The HttpService component and interface handle HTTP connectivity, setting up OAuth from the enviro-
-mental variables. The values of the authentication/authorization keys are generated on the Twitter
Developer dashboard.
...
These can be considered as as the username and password that represents a Twitter developer app when
making API requests.
...
###### Controller
The controller later handles cli interface first line of validations, extracting the relevant argu-
-ments from the command line, casting them or transforming them into the correct data structure to 
...
All Tweet's model property have been declared with primitives wrapper classes for two purpose:
...
- put to work Java standard library's utilities for primive wrapper classes, especially when it 
...
like the `created_at` field, returned as one string, but here modelized as a `ZonedDateTime` class
...
this will allow to clarify the business logic when it comes to optional values or successful API
...
- Use more lambdas and streaming, as Twitter API respones could be significantly large, and using
...
their processing can profit from parallelizing the tasks.