...
-- Helper functions
-- Rounds timestamps to intervals of 5 minutes
...
-- Computes used_memory in percents. (used memory = total memory - free memory)
CREATE FUNCTION used_memory_pcnt(total_mem BIGINT, free_mem INTEGER) RETURNS REAL AS
    $$
BEGIN
    RETURN ((total_mem::DOUBLE PRECISION - free_mem) / total_mem) * 100;
END;
    $$
LANGUAGE PLPGSQL;
END;
...
FROM (
         SELECT u.host_id,
                ...
                OVER( PARTITION BY u.host_id, round_ts_5min(u.timestamp) ) AS avg_used_memory_percent
...
-- Here is a query to detect host failures based on that assumption (< 3 data point means host failure)
